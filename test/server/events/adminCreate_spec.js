...
describe('GET /api/events/:id/admin', () => {
  ...
  it('should return a 401 response without a token', done => {
    api
      .post(`/api/events/${newEvent._id}/admin`)
      .end((err,res) => {
  ...
  it('should return valid event object with the new admin added', done => {
    api
      .post(`/api/events/${newEvent._id}/admin`)
      ...
      .end((err, res) => {
        ...
        expect(res.body.admin[0]._id).to.eq(`${newUser._id}`);
        expect(res.body.admin[0].preferences).to.deep.eq(newUser.preferences);
        expect(res.body.admin[0].username).to.eq(newUser.username);
        expect(res.body.admin[0].email).to.eq(newUser.email);
        expect(res.body.admin[0].password).to.eq(newUser.password);
        done();
      });
  });
});
